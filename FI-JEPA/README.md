# FI-JEPA: executable synthetic baseline

**Current repository status: executable baseline — M1 / E1**

FI-JEPA is an **unvalidated research prototype** for testing whether a joint-embedding predictive objective can learn useful representations from multivariate financial time series. This package does not provide evidence that FI-JEPA is novel, superior to simpler methods, profitable, or suitable for live trading.
...
- a GitHub Actions lane that installs, tests, and reproduces the bounded synthetic report.

This is intentionally a CPU baseline. It establishes a reproducible experiment contract before introducing transformers, real datasets, or paper claims.
...
M1/E1 means only that an executable implementation and deterministic internal verification path exist. It does **not** mean the model has been validated on real point-in-time market data, externally reproduced, externally reviewed, or shown to generate alpha. The repository retains no claim of real-market performance or profitability.

## Required next gate

Before promotion beyond M1/E1 or any manuscript claim, add licensed point-in-time market data, asset-universe and survivorship controls, walk-forward evaluation, multiple seeds, linear and autoregressive baselines, ablations for horizon and EMA momentum, collapse diagnostics, transaction-cost-free representation metrics, raw machine-readable outputs with commit/config provenance, and confidence intervals. Any trading study must remain separate from representation pretraining and must not be presented as financial advice.

Project status is tracked in `../registry/projects.json`; promotion must remain evidence-gated.
